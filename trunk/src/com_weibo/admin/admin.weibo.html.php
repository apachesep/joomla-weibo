...
class HTML_weibo {
    ...
    function showSinaAuth() {
        ...
        $result = $db->loadAssoc();
        ?>
        新浪微博的认证，需要您输入从新浪申请的Appkey 和 AppSecret<p />
        <form action="<?php echo JRoute::_('index.php', true); ?>"  method="post" >
App Key: <input type="text" name="sinaappkey" value="<?php echo $result['oauth_token'] ?>" /><br />
App Secret: <input type="text" name="sinasecret" value="<?php echo $result['oauth_token_secret'] ?>" /><br />
            <input type="submit" />
            <input type="hidden" name="option" value="com_weibo" />
            <input type="hidden" name="task" value="sinaauth2" />
        </form>
