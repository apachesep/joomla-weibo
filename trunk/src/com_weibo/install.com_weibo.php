...
// 输出结果
    ...
    function uninstall($parent) {
        ...
        $db = & JFactory::getDBO();
        $sql = 'SELECT extension_id FROM `#__extensions` WHERE `element` = "weibo" and `type` = "plugin" and `folder`= "authentication";';
        $db->setQuery($sql);
	...
	if ( $result ) {
        ...
        $sql = 'SELECT extension_id FROM `#__extensions` WHERE `element` = "mod_weibologin" and `type` = "module";';
        $db->setQuery($sql);
        $result = $db->loadResult();
	if ( $result ) {
                $installer = new JInstaller();
        ...
        $sql = 'SELECT extension_id FROM `#__extensions` WHERE `element` = "weibo" and `type` = "plugin" and `folder`= "content";';
        $db->setQuery($sql);
        $result = $db->loadResult();
	if ( $result ) {
                $installer = new JInstaller();
