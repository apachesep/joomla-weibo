...
class ONAuthSignatureMethod_HMAC_SHA1 extends ONAuthSignatureMethod
{
    ...
    public function build_signature($request, $consumer, $token)
    {
        ...
        $key_parts = ONAuthUtil::urlencode_rfc3986($key_parts);
...
class ONAuthRequest
{
    ...
    public static function from_request($http_method=NULL, $http_url=NULL, $parameters=NULL)
    {
        ...
        if (!$parameters)
        {
            ...
            $parameters = ONAuthUtil::parse_parameters($_SERVER['QUERY_STRING']);
    ...
    public static function from_consumer_and_token($consumer, $token, $http_method, $http_url, $parameters=NULL)
    {
        ...
        $defaults = array("oauth_version" => ONAuthRequest::$version,
            ...
            "oauth_timestamp" => ONAuthRequest::generate_timestamp(),
    ...
    public function get_signable_parameters()
    {
        ...
        return ONAuthUtil::build_http_query($params);
    }
    ...
    public function get_signature_base_string() {
        ...
        $parts = ONAuthUtil::urlencode_rfc3986($parts);
    ...
    public function to_header()
    {
        ...
        foreach ($this->parameters as $k => $v)
        {
            ...
            $out .= ',' . ONAuthUtil::urlencode_rfc3986($k)
                    . '="' . ONAuthUtil::urlencode_rfc3986($v) . '"';
        }
        return $out;
    }
...
class ONAuthServer
{
    ...
    private function get_consumer(&$request) {
        ...
        if (!$consumer_key) {
            throw new ONAuthException("Invalid consumer key");
        }

        $consumer = $this->data_store->lookup_consumer($consumer_key);
        if (!$consumer) {
            throw new ONAuthException("Invalid consumer");
        }
    ...
    private function check_signature(&$request, $consumer, $token) {
        ...
        if (!$valid_sig) {
            throw new ONAuthException("Invalid signature");
        }
    }
...
class ONAuthUtil
{
    ...
    public static function build_http_query($params)
    {
        ...
        $values = ONAuthUtil::urlencode_rfc3986(array_values($params));
        $params = array_combine($keys, $values);
...
class ONAuth
{
    ...
    function getRequestToken()
    {
        ...
        $this->token = new ONAuthConsumer($token['oauth_token'], $token['oauth_token_secret']);
        return $token;
    }
