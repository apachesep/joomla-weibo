...
// 获取last_key
    ...
    if ($last_key) {
        ...
        $rtn['user_name'] = $user['screen_name'];
        return $rtn;
    } else {
        return $las_tkey;
    }
...
class WeiboClient {
    ...
    function is_followed($uid_or_name) {
        ...
        if (is_numeric($uid_or_name))
            $param['target_id'] = $uid_or_name;
        else
            $param['target_screen_name'] = $uid_or_name;
...
class WeiboOAuth {
    ...
    function getRequestToken($oauth_callback = NULL) {
        ...
        $request = $this->SinaOAuthRequest($this->requestTokenURL(), 'GET', $parameters);
    ...
    function getAccessToken($oauth_verifier = FALSE, $oauth_token = false) {
        ...
        $request = $this->SinaOAuthRequest($this->accessTokenURL(), 'GET', $parameters);
        $token = SinaOAuthUtil::parse_parameters($request);
        $this->token = new SinaOAuthConsumer($token['oauth_token'], $token['oauth_token_secret']);
        return $token;
    }
    ...
    /**
     * GET wrappwer for SinaOAuthRequest. 
     * 
     * @return mixed 
     */
    function get($url, $parameters = array()) {
        $response = $this->SinaOAuthRequest($url, 'GET', $parameters);
    ...
    /**
     * POST wreapper for SinaOAuthRequest. 
     * 
     * @return mixed 
     */
    function post($url, $parameters = array(), $multi = false) {

        $response = $this->SinaOAuthRequest($url, 'POST', $parameters, $multi);
        if ($this->format === 'json' && $this->decode_json) {
            return json_decode($response, true);
        }
        return $response;
    ...
    function delete($url, $parameters = array()) {
        $response = $this->SinaOAuthRequest($url, 'DELETE', $parameters);
        if ($this->format === 'json' && $this->decode_json) {
            return json_decode($response, true);
        }
        return $response;
     ...
     * @return string 
     */
    function SinaOAuthRequest($url, $method, $parameters, $multi = false) {
        ...
        switch ($method) {
            case 'GET':
                $e = $this->http($request->to_url(), 'GET');
                return $e;
            default:
