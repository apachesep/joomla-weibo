...
function getWeiboText($row, $P, &$weibocontent) {
    ...
    $weibocontent['imgurl'] = JRoute::_($picurl, true);
}
...
function plgWeibo(&$row, $isNew) {
    ...
    // 参数的设置
    $P['sinaenabled'] = $pluginParams->get('sinaenabled', false); //是否启用新浪微博
    if ($P['sinaenabled']) {
    ...
    $P['tencentenabled'] = $pluginParams->get('tencentenabled', false); //是否启用腾讯微博
    if ($P['tencentenabled']) {
    ...
    $P['neteaseenabled'] = $pluginParams->get('neteaseenabled', false); //是否启用网易微博
    if ($P['neteaseenabled']) {
    ...
    $P['weibotype'] = $pluginParams->get('weibotype', 'fulltext'); // 微博发表方式（fulltext，onlytitle，introtext或者custom）
    $P['catid'] = $pluginParams->get('catid'); // 所指定的分类
    $P['customstring'] = $pluginParams->get('customstring'); // 自定义的字符串
    $P['picsend'] = $pluginParams->get('picsend', true); // 将文章中的第一幅图片发布到微博上
